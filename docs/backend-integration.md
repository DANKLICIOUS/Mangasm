...
| `ReputationService.score(for:)/canViewPhotos(viewerScore:targetGate:)` / `selectStyle` | Live: `my_profile_style()` + `UPDATE profiles.selected_style_id`. `unlocked_style_ids` is write-time SoT. Unlock map: new &lt;40, building 40+, reliable 65+, verified 85+. **Do not db-push this repo’s `supabase/` onto the backend project.** See `docs/reputation-live.md` and mangasm-backend `docs/PROFILE_STYLE.md`. Photo gate remains score vs `photo_gate` (not style). |
